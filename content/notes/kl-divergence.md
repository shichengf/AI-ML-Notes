...
两个归一化分布之间真正的 KL 散度不会小于零。但是，某个 $x$ 对 KL 的局部贡献可以是负数：
...
这些正负不一的局部贡献只有在完整求期望以后，才构成 KL 散度。

> [!warning] 真正的 KL 与代码报告的数值
> 真正的 KL 不会是负数。单点贡献、带噪声的估计，或者与 KL 相差一个常数的目标函数都可能是负数。
...
遇到明显的负数时，不应该直接归因于数值误差。较小的 Monte Carlo 负估计可能合理，但闭式 KL 公式通常不应小于零，除非存在很小的浮点数误差。
