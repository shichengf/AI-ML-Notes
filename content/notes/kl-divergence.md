...
| 符号 | 它表示什么 |
| --- | --- |
| $D_{\mathrm{KL}}$ | $D$ 表示 divergence，也就是两个分布之间的差异。下标 $\mathrm{KL}$ 是这个散度的名字，不是参与计算的变量。 |
| $P\parallel Q$ | 读作“$P$ 相对于 $Q$”。$P$ 放在前面，$Q$ 放在后面。交换顺序通常会得到不同结果。$\parallel$ 只是分隔两个分布，不表示绝对值或条件概率。 |
| $P$ 与 $p(x)$ | 大写 $P$ 表示整个分布。小写 $p(x)$ 表示分布 $P$ 给某个具体结果 $x$ 的概率或概率密度。$Q$ 与 $q(x)$ 的关系相同。 |
| $x$ | 随机变量的一种可能取值。例如，掷骰子时，$x$ 可以是 1 到 6 中的一个数。 |
| $\sum_x$ | 遍历 $x$ 的所有可能取值，再把每一项相加。写在求和符号下方的 $x$ 说明“对谁求和”。 |
| $\log\frac{p(x)}{q(x)}$ | 比较 $P$ 和 $Q$ 对同一个 $x$ 给出的概率。两者越接近，这一项越接近零。 |
| $\mathbb{E}_{x\sim P}$ | 对所有结果求加权平均。下标 $x\sim P$ 表示 $x$ 按照 $P$ 分布，也表示平均时由 $P$ 提供权重。 |
| $Q_\theta$ 或 $q_\phi$ | $\theta$ 和 $\phi$ 表示控制分布的模型参数。训练模型就是调整这些参数。 |
| $P_{\mathrm{data}}$ | 下标 $\mathrm{data}$ 是角色标签，说明这是数据分布。它不是乘法。$P_{\mathrm{teacher}}$ 等写法也一样。 |
| $q_\phi(z\mid x)$ | 在给定 $x$ 的条件下，$z$ 的分布由参数 $\phi$ 控制。竖线 $\mid$ 表示“在某个条件下”。 |

> [!tip] 一句话读法
> 按照 $P$ 出现各种 $x$ 的频率，比较 $P$ 和 $Q$ 给同一个 $x$ 的概率，然后把这些比较结果取平均。
